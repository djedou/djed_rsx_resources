...
use djed_rsx_shared::consts::{DEFAULT_FONT_DPI, DEFAULT_FONT_SIZE};
use djed_rsx_shared::traits::{
    font_traits::{TEncodedFont, TFontCache},
    key_traits::{ TFontInstanceKey, TFontKey, TFontKeysAPI, TGlyphInstance, TGlyphStore}
};
use uuid::Uuid;
...
pub use djed_rsx_shared::types::font_types::{FontEncodedData, FontInstanceResourceData, FontResourceData};
    ...
    A: TFontKeysAPI + 'static
{
    ...
    fn shape_text_h<T>(&self, _instance: &Self::FontInstance, _text: T) -> Option<Self::Glyphs>
    where
        T: AsRef<str>
    {
    ...
    fn shape_text_v<T>(&self, _instance: &Self::FontInstance, _text: T) -> Option<Self::Glyphs>
    where
        T: AsRef<str>
    {
    ...
    A: TFontKeysAPI
{
    ...
    pub fn add_font<E>(&mut self, _font_id: FontId, encoded: &E, face_index: usize) -> Result<()>
    where
        E: TEncodedFont
    {
        let _decoded = DecodedFont::from_encoded_font(encoded, face_index);
        //self.context.add_face(font_id, &decoded.bytes, face_index)?;

        //let family_name = self.context.get_family_name(font_id)?;
        let _size = DEFAULT_FONT_SIZE;
        let _dpi = DEFAULT_FONT_DPI;
        //let font_instance_id = FontInstanceId::from_family_str(family_name, size, dpi);
    ...
    pub fn get_family_name_for_id(&self, _id: FontId) -> Result<String> {
    ...
    pub fn get_bytes(&self, _font_id: FontId) -> Result<Rc<Vec<u8>>> {
        //self.context.get_bytes(font_id)
        Ok(Rc::new(Vec::new()))
    }

    pub fn get_face_index(&self, _font_id: FontId) -> Result<()>/*Result<usize>*/ {
        //self.context.get_face_index(font_id)
        Ok(())
    }

    pub fn get_family_name(&self, _font_id: FontId) -> Result<&str> {
        //self.context.get_family_name(font_id)
        Ok("")
    }

    pub fn get_glyph_index(&self, _instance: FontInstanceRef<A>, _c: char) -> Result<u32> {
        //self.context.get_glyph_index(instance, c)
        Ok(0)
    }

    pub fn get_glyph_dimensions(&self, _instance: FontInstanceRef<A>, _c: char) -> Result<()>/*Result<GlyphDimensions>*/ {
        //self.context.get_glyph_dimensions(instance, c)
        Ok(())
    }

    pub fn get_global_size_metrics(&self, _instance: FontInstanceRef<A>) -> Result<()>/*Result<FontSizeMetrics>*/ {
    ...
    pub fn shape_text_h<T>(
        &self,
        _instance: FontInstanceRef<A>,
        _text: T
    ) -> Result<()>/*Result<GlyphStore<A::FontKey, A::FontInstanceKey, A::GlyphInstance>>*/
    where
        T: AsRef<str>
    {
