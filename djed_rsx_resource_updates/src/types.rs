...
use djed_rsx_shared::traits::key_traits::{TFontInstanceKey, TFontKey, TFontKeysAPI, TGlyphInstance, TImageKeysAPI, TMediaKey};
use djed_rsx_shared::types::{
    font_types::{FontEncodedData, FontInstanceResourceData, FontResourceData},
    image_types::{ImageEncodedData, ImageResourceData}
};
use serde::Serialize;
use serde_json;
...
impl TImageKeysAPI for DefaultImageKeysAPI {
    ...
    fn add_image(&mut self, encoded: ImageEncodedData, _: ImageResourceData) -> Self::ImageKey {
        ...
        let uri = match encoded {
            ImageEncodedData::Bytes { format, bytes } => Rc::new(djed_base64_util::to_image_data_uri(format.as_ref(), bytes)),
...
impl TFontKeysAPI for DefaultFontKeysAPI {
    ...
    fn add_font(&mut self, encoded: FontEncodedData, _: FontResourceData) -> Self::FontKey {
        ...
        let uri = match encoded {
            FontEncodedData::Bytes { bytes } => Rc::new(djed_base64_util::to_font_data_uri(bytes)),
